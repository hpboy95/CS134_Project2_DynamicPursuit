...
void SpriteList::update() {
	...
	for (int i = 0; i < sprites.size(); i++) {
		sprites[i].getPos() += sprites[i].velocity / ofGetFrameRate();
	}
}
...
void Emitter::draw() {
	if (drawable) {
		...
		if (haveImage) {
			image.draw(-image.getWidth() / 2.0 + getPos().x, -image.getHeight() / 2.0 + getPos().y);
		}
		else {
			ofSetColor(0, 0, 200);
			ofDrawRectangle(-width / 2 + getPos().x, -height / 2 + getPos().y, width, height);
		}
	}
...
void Emitter::moveSprite(Sprite *sprite) {
    sprite->setPos(sprite->getPos() + sprite->velocity / ofGetFrameRate());
}
...
void Emitter::spawnSprite() {
	Sprite sprite;
	sprite.verts.push_back(glm::vec3(-50, 50, 0));
	sprite.verts.push_back(glm::vec3(50, 50, 0));
	sprite.verts.push_back(glm::vec3(0, -50, 0));
