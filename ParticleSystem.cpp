...
// Radial Force - this is a "one shot" force that
// eminates radially outward in a ring.
//
RadialForce::RadialForce(float magnitude, float thickness) {
	this->magnitude = magnitude;
...
void RadialForce::updateForce(Particle* particle) {
	...
	// the force is only added once after it is triggered.
	//
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(-thickness, thickness), ofRandom(-1, 1));
	particle->forces += dir.getNormalized() * magnitude;
}

void RadialForce::setThickness(float thickness) {
	this->thickness = thickness;
}

// Cyclic Force - this is a "one shot" force that
// Eminates particles in a vortex
//
CyclicForce::CyclicForce(float magnitude, float rotation) {
